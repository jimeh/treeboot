...
if [[ "$(wc -l <"${registry_tests}")" -ne 316 ]]; then
  printf 'treeboot spec cases: expected 316 unique generated registry source keys\n' >&2
...
printf 'treeboot spec cases: 316 audited cases, 20 closure cases, 18 reference-only tests, full conformance drivers present\n'
